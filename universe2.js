;(function (window) {
    ...
    let texts = ['HEYY :)','HAPPY BIRTHDAY', 'HEHEHE', 'COBA LIHAT DEH', 'LIHAT LANGITNYA', 'CANTIK YAA', 'BANYAK KUNANG-KUNANG', 'ADA BINTANG JATUH JUGA', 'AKU MAUU','DI HARI INI', 'DI SINIII', 'KITA MAKE A WISH', 'AKU DULU YAA', 'SEMOGA', 'APAPUN YANG KAMU MAU','SEMUA HARAPAN KAMU', 'KEINGINAN KAMU', 'IMPIAN KAMUU', 'YANG TERBAIK BUAT KAMUU', 'SEMUANYA TERCAPAI', 'SEKARANG GILIRAN KAMU', 'YANG MAKE A WISH', 'UDAAH??', 'SEMOGA SEMUA DOA KAMU', 'DIKABULKANN','AAMIIN', 'TERAKHIR NII','HAPPY', 'BIRTHDAY', 'OLEENN :)']
    ...
    function draw () {
      ...
      let lineHeight = textSize * 1.2; // Sesuaikan jarak antar baris

    for (let i = 0; i < lines.length; i++) {
    ctx.fillText(
        lines[i], 
        (CANVASWIDTH - ctx.measureText(lines[i]).width) * 0.5, 
        (CANVASHEIGHT * 0.5) + (i - (lines.length - 1) / 2) * lineHeight
    );
}
